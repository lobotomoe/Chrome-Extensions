...
window.addEventListener("message", function (event) {
  if (event.source != window || !event.data.messageFromContentScript1234)
    return;
    ...
    startRecording: function (options, callback) {
      ...
      if (options) {
        ...
        if (!Object.keys(options).length) {
          callback(
            false,
            "First parameter must specify at least one recoding type."
          );
          return;
        }
        ...
        if (mismatchedValues.length) {
          callback(
            false,
            "Unsupported parameters detected: " + mismatchedValues.join(", ")
          );
          return;
        }
...
(document.head || document.body || document.documentElement).appendChild(
  script
).text = RecordRTC_Extension.toString() + ";;";
script.remove();
